...
const Dashboard: React.FC = () => {
  ...
  const { entries: moodEntries } = useAppSelector((state: RootState) => state.mood);
  const { entries: journalEntries } = useAppSelector((state: RootState) => state.journal);
  const { conversations } = useAppSelector((state: RootState) => state.conversation);
  ...
  const activeConversations = conversations.slice(0, 3); // Remove isActive filter since we don't have it
