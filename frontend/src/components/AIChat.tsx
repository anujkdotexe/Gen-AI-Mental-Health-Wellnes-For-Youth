...
import { createConversation, sendMessage, fetchConversations, fetchConversationDetails, setActiveConversation, type Message, type Conversation } from '../store/slices/conversationSlice';
...
const AIChat: React.FC = () => {
  ...
  useEffect(() => {
    if (activeConversation?.messages && Array.isArray(activeConversation.messages)) {
      // Filter out any null/undefined messages and ensure proper structure
      ...
      setCurrentMessages(validMessages);
    } else {
      setCurrentMessages([]);
    }
  }, [activeConversation]);
  ...
  const handleSelectConversation = async (conversation: Conversation) => {
    // If this conversation is already active, do nothing
    if (activeConversation?.id === conversation.id) {
      return;
    }

    // If the conversation already has messages loaded, just set it as active
    if (conversation.messages && conversation.messages.length > 0) {
      dispatch(setActiveConversation(conversation));
    } else {
      // Fetch the full conversation details with messages
      try {
        await dispatch(fetchConversationDetails(conversation.id));
      } catch (error) {
        ...
        <div className="bg-white rounded-lg shadow-md h-full flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {!activeConversation ? (
              ...
              <>
                {currentMessages.filter(message => message && typeof message === 'object').map((message, index) => (
                  <div
                  ...
                  >
                    ...
                    >
                      ...
                      <p className={`text-xs mt-1 ${
                        message.is_user ? 'text-purple-200' : 'text-gray-500'
                      }`}>
